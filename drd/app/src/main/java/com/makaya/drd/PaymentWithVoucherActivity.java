...
public class PaymentWithVoucherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.payment_with_voucher);
        activity=this;
        /*PublicFunction2.setStatusBarColor(this, R.color.colorBased);
        TextView title=(TextView)findViewById(R.id.appBarTitle);
        title.setText("PAYMENT WITH VOUCHER");
        title.setVisibility(View.VISIBLE);*/
        PublicFunction.setHeaderStatus(activity,"Payment With Voucher");
        ...
        voucherNo = (EditText) findViewById(R.id.voucherNo);
        ...
        text=text.replace("[_AMOUNT_]","<b>DRD Point "+NumberFormat.getInstance().format(dataTrxPayAmount)+"</b>");
    ...
    void submitPayment()
    {
        if (voucherNo.getText().toString().trim().equals("")){
            Toast.makeText(activity,"Voucher number required.",Toast.LENGTH_SHORT).show();
            voucherNo.requestFocus();
            return;
        }
        ...
        if (dataTrxType.contains("TU")){
            ...
            pay.VoucherNo=voucherNo.getText().toString();
            ...
            srv.setOnDataPostedListener(new MemberTopupPaymentService.OnSetDataPostedListener() {
                ...
                public <T> void onDataPosted(T data, boolean isLoadedAllItems) {
                    long val=((MemberTopupPayment)data).Id;
                    if (val<0) {
                        btnConfirm.setEnabled(true);
                        btnConfirm.setTextColor(Color.WHITE);
                        btnConfirm.setText("CONFIRM");
                        if (val == -1)
                            Toast.makeText(activity, "Invalid voucher number.", Toast.LENGTH_SHORT).show();
                        else if (val == -2)
                            Toast.makeText(activity, "The nominal amount is not the same as the amount to be paid.", Toast.LENGTH_SHORT).show();
                        return;
                    }
    ...
    {
        payName.setText(method.Bank.Name);

        String path=MainApplication.getUrlApplWeb()+"/Images/bank/"+method.Bank.Logo;
